...
module.exports.createController = function (config, Db/*, signer*/) {
  ...
  // Controller
  //
  OauthClients.login = function (config, key, secret/*, opts*/) {
    return Db.ApiKeys.get(authutils.hashsum('sha256', key)).then(function (apikey) {
        var err;
  ...
  OauthClients.createKeys = function (config, client, raw, manualOpts) {
    ...
    // TODO JWT as key
    if (raw.test) {
      keypair.key = raw.key || ('TEST_ID_' + getId(16, 24));
      if (!keypair.secret) {
        keypair.secret = raw.secret || ('TEST_SK_' + getSecret(16, 24));
      }
    }
    else if (manualOpts && manualOpts.forceIds) {
      if (!keypair.id) {
        keypair.key = 'ID__' + getId(16, 28);
      }
      if (!keypair.secret) {
        keypair.secret = raw.secret || 'SK__' + getSecret(16, 28);
      }
    }
    else {
      keypair.key = 'ID__' + getId(16, 28);
