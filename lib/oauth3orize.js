...
module.exports.create = function (config, TokenStore, ClientsCtrl, LoginsCtrl, Signer) {
  ...
  function exchangePasswordToken(auth, params) {
    ...
    , acs: (login.accountIds||[]).map(function (a) { return a && { id: a.id }; })
    ...
    , axs: (login.accountIds||[]).map(function (a) {
  ...
  // grant_type=['client_credentials', 'password', 'delegated', 'authorization_code', 'implicit', 'refresh_token', (code??)]
  // response_type=['code', 'token', (authorization_code??)]
  function parseResourceOwnerPassword(req/*, opts*/) {
    ...
    if (!params) {
      ...
      err = new Error("grant_type 'password' is missing 'password' parameter (misnomer: password refers to the secret, passphrase, or other authentication token)");
      err.code = 'E_MISSING_PARAM';
      return PromiseA.reject(err);
    }
  ...
  function parseRegisterClient(req/*, opts*/) {
    ...
    console.log('DEBUG registerClient token');
    console.log(token);
  ...
  function exchangeRefreshToken(params/*, opts*/) {
    return Signer.verifyAsync(params.experienceId || params.origin, params.refreshToken).then(function (tokenMeta) {
      ...
      //tokenMeta.scope = params.scope.join(' ');
      ...
      return ClientsCtrl.login({ /*tosUrl: ''*/ }, params.clientId, null, params).then(function (apiKey) {
        ...
        return LoginsCtrl.rawAccountIds({
          ...
          tokenMeta.axs = (accounts||[]).map(function (a) {
  ...
  function registerClient(params/* opts*/) {
    return ClientsCtrl.exists({ /**/ }, params.clientUri, null, params).then(function (exists) {
      if (exists) {
        // Allow re-registration? dual registrations? (multiple issuers / azps?)
        return PromiseA.reject(new Error("this client is already registered"));
      }
      ...
      */
      ...
      return requestJson({ url: url }).then(function (resp) {
        ...
        // TODO check jwks_uri against iss
        return requestJson({ url: keysUrl }).then(function (resp) {
          // http://connect2id.com/products/server/docs/config/jwk-set
          if (!(resp.data && Array.isArray(resp.data.keys) && resp.data.keys.length)) {
            return PromiseA.reject(new Error("no array of keysclient key checking not implemented"));
          }
          var keys = resp.data.keys;
          // TODO find key by params.token.kid (perhaps use shasum of pubkey)
          var key = keys[0];

          // verify token
          var jwt = PromiseA.promisifyAll(require('jsonwebtoken'));
          return jwt.verifyAsync(params.encodedToken, key.toPublicPem()).then(function () {
            return PromiseA.reject(new Error("client key checking not implemented"));
            return ClientsCtrl.createHelper(null, params.clientUri);
          });
        });
      });

      /*
      var err = new Error("register client not implemented");
      err.code = 'E_NO_IMPL';
      return PromiseA.reject(err);
      */
    });
