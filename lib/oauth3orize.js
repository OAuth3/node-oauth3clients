...
module.exports.create = function (config, TokenStore, ClientsCtrl, Logins, Signer, weakCipher) {
  ...
  function exchangePasswordToken(auth, params) {
    ...
    , idx: weakCipher(oauthClient.secret, login.id || login.hashId)               // loginId
    , acx: login.primaryAccountId && {                                            // selectedAccountId
              id: weakCipher(oauthClient.secret, login.primaryAccountId)
            }
    , aax: (login.accounts||[]).map(function (a) {
              return a && { appScopedId: weakCipher(oauthClient.secret, a.id) };
            })
