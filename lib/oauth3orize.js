      ...
      */
      ...
      return requestJson({ url: url }).then(function (resp) {
        ...
        return requestJson({ url: keysUrl }).then(function (resp) {
          ...
          var key = keys[0];
          var pubPem = require('ursa').createPublicKeyFromComponents(
            new Buffer(key.n, 'base64') // modulus
          , new Buffer(key.e, 'base64') // exponent
          );

          // verify token
          var jwt = PromiseA.promisifyAll(require('jsonwebtoken'));
          return jwt.verifyAsync(params.encodedToken, pubPem.toPublicPem()).then(function () {
