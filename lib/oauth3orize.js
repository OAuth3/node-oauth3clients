...
module.exports.create = function (config, TokenStore, ClientsCtrl, LoginsCtrl, Signer, myRootClient) {
  ...
  function exchangeRefreshTokenHelper(params, refreshToken, tokenMeta, axs) {
    ...
    tokenMeta = JSON.parse(JSON.stringify(tokenMeta));
    ...
    tokenMeta.ip = params.ip;                 // TODO check IP as a session, require revalidation when ip changes
    ...
    tokenMeta.axs = axs;
    tokenMeta.sub = axs.map(mapSubIds).join(',');
  ...
  function exchangeAuthorizationCode(params/*, opts*/) {
    ...
    , params
      ...
      return Signer.verifyAsync(params.experienceId, params.code).then(function (codeMeta) {
        ...
        , login: null
        , ppid: codeMeta.sub
        , accounts: codeMeta.axs || codeMeta.acx && [codeMeta.acx] || [{
            id: codeMeta.ppid || codeMeta.sub
          }]
        };
        /*
        ...
        */
        ...
        return createRefreshToken(auth, params, axs, 'code').then(function (refreshResult) {
