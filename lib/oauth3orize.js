...
module.exports.create = function (config, TokenStore, ClientsCtrl, LoginsCtrl, Signer, myRootClient) {
  ...
  function getClient(params/*, opts*/) {
  ...
  function getClientAndUser(params/*, opts*/) {
    ...
    return ClientsCtrl.loginOrCreate({ /*tosUrl: ''*/ }, params.clientId, params.clientSecret, params).then(function (apiKey) {
      ...
      , mfa: params.mfa || undefined
      , totp: params.totp || ''
      , otp: params.otp || undefined
      , otpUuid: params.otpUuid || undefined
      // , tenantId:  params.tenantId
        ...
        , login: login
        , newCredential: login.newCredential
        };
      });
    }, function (err) {
  ...
  function exchangeRefreshTokenHelperHelper(params, tokenMetaOriginal, axs, expiresIn) {
    ...
    tokenMeta.jti = crypto.randomBytes(16).toString('hex');
  ...
  function exchangeRefreshTokenHelper(params, refreshToken, tokenMeta, axs) {
    ...
    //console.log('tokenMetaCopy abcdef', tokenMetaCopy);
  ...
  function exchangeRefreshToken(params/*, opts*/) {
    return Signer.verifyAsync(params.experienceId || params.origin, params.refreshToken).then(function (tokenMeta) {
      //console.log('tokenMeta abcdef', tokenMeta);
      ...
      return ClientsCtrl.login({/*tosUrl: ''*/}, params.clientId, null, params).then(function (apiKey) {
        ...
        return LoginsCtrl.rawAccountIds({
          id: scoper.unscope(tokenMeta.idx || tokenMeta.sub, oauthClient.secret)
          ...
          */
          ...
          // TODO check params.ip, tokenMeta.srv
  ...
  function parseResourceOwnerPassword(req/*, opts*/) {
    ...
    , otp: params.otp // for Login Code (via Email or Text)
    , otpUuid: params.otp_uuid || params.otpUuid // for Login Code (via Email or Text)
  ...
  function parseLoginCode(req/*, opts*/) {
    var params = req.body;
    var err;

    console.log('parseLoginCode req.body');
    ...
    , requestOtp: params.request_otp || params.requestOtp // for Login Code (via Email or Text)
  ...
  function parseRefreshToken(req/*, opts*/) {
    ...
    // The question arises: Why don't we cheat by keeping client_id like this?
    // Answer: we can't distinguish between the app oauth3.org and oauth3.org/my-app/subpage/ otherwise
    ///*
    params.client_id = params.client_id || params.clientId
      || params.client_uri
      || req.headers.referer
      || req.headers.origin
      ;
